...
import { getPathsParameters2, getRequestBody2, getResponses2 } from './newSwagger';
          ...
          if (schemaRef.length > 0) {
            schemaRef.forEach(async (schema) => {
              ...
              if (Object.values(schemaRequiredParameters).length > 0) {
                preCondition200[path][method][schemaName] = schemaRequiredParameters;
              } else {
                ...
                preCondition200[path][method][index] = newObject;
              }
      ...
      if (Object.values(resquestBody[path][method]).length > 0) {
        ...
        for (const key in preCondition200[path][method]) {
          ...
          for (const parameterName in preCondition200[path][method][key]) {
            if (typeof preCondition200[path][method][key][parameterName] === 'object') {
              for (const propName in preCondition200[path][method][key][parameterName]) {
                newObject[key][index] = {
...
export const createPreCondition404 = async (swaggerFile: SwaggerFile) => {
  ...
  return `${specificUrl[0].url}${pathValue[0]}/404`;
};
            ...
            for (const parameterName in preCondition200[path][method][key]) {
              if (typeof preCondition200[path][method][key][parameterName] === 'object') {
                for (const propName in preCondition200[path][method][key][parameterName]) {
                  newObject[key][index] = {
                    ...preCondition200[path][method][key][parameterName],
                    [propName]: preCondition200[path][method][key][parameterName] + '*%5r'
                  };
                  index++;
                }
              } else {
                if (preCondition200[path][method][key][parameterName]) {
                  newObject[key][index] = {
                    ...preCondition200[path][method][key],
                    [parameterName]: preCondition200[path][method][key][parameterName] + '*%5r'
                  };
                  index++;
                }
              }
